...
public class Generate {
	...
	public static void main(String[] args) throws Exception {
		...
		JdbcConfig jdbcConfig = new JdbcConfig(po.getProperty("jdbc.driver"), po.getProperty("jdbc.url"),
				po.getProperty("jdbc.username"), po.getProperty("jdbc.password"));
		...
		// 单张表生成代码
		String moduleName = "bussiness"; // 模块名，例：sys
		String subModuleName = ""; // 子模块名（可选）
		String className = "GoodsConsumer"; // 类名，例：user
		String functionName = "对账"; // 功能名，例：用户
		String tablename = "tab_goods_consumer"; // 表名

		GenCode(datebase, dbProvider, packageName, moduleName, subModuleName, className, classAuthor, functionName,
				tablename);

	}

	public static void GenCode(String datebase, MysqlProvider dbProvider, String packageName, String moduleName,
			String subModuleName, String className, String classAuthor, String functionName, String tablename)
			throws IOException {
		...
		String templatePath = classPath.replace(separator + "WebRoot" + separator + "WEB-INF" + separator + "classes",
				separator + "src" + separator + "com" + separator + "whty" + separator + "platform" + separator
						+ "modules");
		...
		cfg.setDirectoryForTemplateLoading(new File(templatePath
				.replace("modules", "generate" + separator + "template")));
		// 从数据库中得到该表的所有字段
		...
		tbmodel.put("viewPrefix", StringUtils.substringAfterLast((String) tbmodel.get("packageName"), ".") + "/"
				+ tbmodel.get("urlPrefix"));
		tbmodel.put("permissionPrefix", tbmodel.get("moduleName")
				+ (StringUtils.isNotBlank(subModuleName) ? ":" + StringUtils.lowerCase(subModuleName) : "") + ":"
				+ tbmodel.get("className"));
		...
		String filePath = javaPath + separator + tbmodel.get("moduleName") + separator + "entity" + separator
				+ StringUtils.lowerCase(subModuleName) + separator + tbmodel.get("ClassName") + ".java";
		...
		filePath = javaPath + separator + tbmodel.get("moduleName") + separator + "dao" + separator
				+ StringUtils.lowerCase(subModuleName) + separator + tbmodel.get("ClassName") + "Dao.java";
		writeFile(content, filePath);
		logger.info(filePath);
		...
		content = FreeMarkers.renderTemplate(template, tbmodel);
		filePath = javaPath + separator + tbmodel.get("moduleName") + separator + "service" + separator
				+ StringUtils.lowerCase(subModuleName) + separator + tbmodel.get("ClassName") + "Service.java";
		writeFile(content, filePath);
		logger.info(filePath);
		...
		content = FreeMarkers.renderTemplate(template, tbmodel);
		filePath = javaPath + separator + tbmodel.get("moduleName") + separator + "web" + separator
				+ StringUtils.lowerCase(subModuleName) + separator + tbmodel.get("ClassName") + "Controller.java";
		writeFile(content, filePath);
		logger.info(filePath);
		...
		content = FreeMarkers.renderTemplate(template, tbmodel);
		filePath = viewPath + separator + StringUtils.substringAfterLast((String) tbmodel.get("packageName"), ".")
				+ separator + tbmodel.get("moduleName") + separator + StringUtils.lowerCase(subModuleName) + separator
